...
class BelongsToMany extends Relation
{
    ...
    /**
     * Adds a WHERE condition on a pivot table column
     *
     * @example ->wherePivot('status', 'accepted')
     * @example ->wherePivot('status', '!=', 'blocked')
     */
    public function wherePivot(string $column, mixed $operator, mixed $value = null): static
    {
        // Suporte a 2 argumentos: wherePivot('col', 'val') == wherePivot('col', '=', 'val')
        if ($value === null) {
            $value = $operator;
            $operator = '=';
        }
    ...
    /**
     * Adds an OR WHERE condition on a pivot table column
     */
