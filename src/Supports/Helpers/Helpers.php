...
if (!function_exists('avatar')) {
    /**
     * Gera um avatar SVG com cor de fundo dinâmica.
     */
    function avatar(string $name, int $size = 40): string {
        // 1. Geramos a cor dinâmica baseada no nome
        $bg = getAvatarColor($name);
        $color = '#ffffff';

        // 2. Extraímos as iniciais
        ...
        // 3. Retornamos o SVG com a cor dinâmica já aplicada no fill
        return <<<SVG
        <svg xmlns="http://www.w3.org/2000/svg" width="{$size}" height="{$size}" viewBox="0 0 {$size} {$size}">
            <circle cx="20" cy="20" r="20" fill="{$bg}"/> 
            <rect width="{$size}" height="{$size}" rx="{$size}" fill="{$bg}"/>
            <text x="50%" y="50%" dominant-baseline="central" text-anchor="middle"
                font-family="Inter, system-ui, sans-serif" font-size="{$font}" font-weight="600" fill="{$color}">
                {$initials}
            </text>
        </svg>
        SVG;
...
/**
 * Gera uma cor HSL persistente para o avatar.
 */
function getAvatarColor(string $string): string 
{
    $hash = md5($string);
